...
프론트는 Vercel, 백엔드는 Render에 올리는 구조다. 도메인이 서로 다르므로
프론트가 백엔드 주소를 환경변수로 받고, 백엔드는 그 도메인을 CORS로 허용한다.

```
브라우저 → Vercel (정적 파일)  ──VITE_API_BASE_URL──→  Render (FastAPI + SQLite)
...
| 변수 | 값 | 설명 |
|---|---|---|
| `VITE_API_BASE_URL` | `https://<render-앱>.onrender.com` | 비워 두면 `/api` 상대경로를 쓴다(로컬 개발). 끝에 슬래시나 `/api`를 붙이지 않는다 |

`frontend/.env.example`에 설명이 있다. 로컬에서 시험하려면 `.env.local`로 복사한다.

**백엔드 (Render)**

| 변수 | 기본값 | 설명 |
|---|---|---|
| `ALLOWED_ORIGINS` | `http://localhost:5173,http://127.0.0.1:5173` | CORS 허용 도메인, 쉼표 구분. Vercel 주소를 넣는다 |
| `PORT` | `8000` | Render가 주입한다 |
| `DEMO_MODE` | `false` | 켜면 화면 상단에 데모 안내를 띄우고 포지션 수에 상한을 건다 |
| `DEMO_MAX_POSITIONS` | `50` | 데모 모드의 포지션 상한 |
| `AUTO_SEED` | `DEMO_MODE`와 같음 | 시작할 때 DB가 비어 있으면 예시 북을 만든다 |
| `TRE_DB_PATH` | `backend/data/trading.db` | DB 파일 경로 |

환경변수를 하나도 주지 않으면 지금까지와 똑같이 로컬 개발 설정으로 동작한다.

### Render

`render.yaml`이 저장소 루트에 있다. 시작 명령은 다음과 같다.

```
uvicorn app.main:app --host 0.0.0.0 --port $PORT
```

Python 버전은 `backend/runtime.txt`와 `render.yaml`에 `3.12.8`로 고정했다.
`ALLOWED_ORIGINS`는 Vercel 주소를 배포한 뒤에 Render 대시보드에서 채운다(미리 알 수 없다).

### Vercel

프로젝트 루트를 `frontend`로 지정하면 `npm run build`와 `dist` 가 자동으로 잡힌다.
환경변수에 `VITE_API_BASE_URL`을 넣고 다시 배포한다.
Vite는 빌드 시점에 값을 번들에 넣으므로, 값을 바꾸면 **다시 배포해야** 반영된다.

### 무료 플랜에서 생기는 일

**백엔드가 잠든다.** 15분쯤 요청이 없으면 Render 무료 플랜은 잠들고, 깨어나는 데
최대 1분 걸린다. 그동안 화면이 비어 보이면 고장으로 오해하기 쉬우므로,
앱이 먼저 `/api/health`를 호출해 깨우고 **"데모 서버를 깨우는 중입니다.
최대 1분 걸릴 수 있습니다"** 를 띄운다. 실패하면 다시 시도 버튼을 준다.
로컬 개발에서는 첫 호출이 즉시 성공하므로 이 화면이 보이지 않는다.

**디스크가 초기화된다.** 재시작하면 SQLite 파일이 사라진다. 그래서 `AUTO_SEED`가
켜져 있으면 시작할 때 DB가 비었는지 보고 예시 북을 다시 만든다.
이미 데이터가 있으면 건너뛴다. 시드는 라우트와 같은 경로를 지나므로
호가단위 검증 같은 규칙이 그대로 적용된다.

### 배포 후 확인할 것

**pykrx는 네이버에서 데이터를 가져온다. 클라우드 IP에서는 차단되거나 느릴 수 있다.**
배포한 뒤 반드시 확인한다.

```
curl https://<render-앱>.onrender.com/api/market/quote/005930
```

- **되면**: 그대로 쓰면 된다.
- **안 되면**: 시세 조회만 실패하고 나머지는 모두 동작한다. 확인할 것은 다음과 같다.
  - 북 현황에 시드 데이터(평균단가, 실현손익, 체결 이력)가 보이는지.
    이 값들은 시세와 무관하게 계산되므로 영향을 받지 않는다.
  - 선물처럼 평가가격을 직접 입력해 평가가 되는지.
  - 포지션 평가 화면에서 평가가격을 손으로 넣어 계산과 근거가 나오는지.
  - 실패 메시지가 이해 가능한지. 예:
    `005930 시세를 가져오지 못했습니다 (pykrx: 데이터 없음; FinanceDataReader: 데이터 없음).`
    두 소스를 모두 시도했다는 사실과 무엇을 하면 되는지가 드러나야 한다.

시세가 막히면 `AUTO_SEED`가 만든 체결은 그대로 쌓이고 평가만 비어 있다.
화면에는 "아직 평가하지 않음"으로 표시되고, 사용자가 평가가격을 넣어 재평가할 수 있다.
